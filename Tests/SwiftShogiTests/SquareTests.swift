...
final class SquareTests: XCTestCase {
    ...
    func testRankCases() {
        ...
        XCTAssertEqual(cases, [.i1, .h1, .g1, .f1, .e1, .d1, .c1, .b1, .a1])
    }

    func testPromotableCases() {
        let colors: [(color: Color, expected: Set<Square>)] = [
            (.black, Set([
        ...
        colors.forEach {
            XCTAssertEqual(Set(Square.promotableCases(for: $0.color)), $0.expected)
        }
    }
}
